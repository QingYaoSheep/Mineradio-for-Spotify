...
const REALTIME_PROPERTY_KEYS = new Map([
  ['inputgain', 'inputGain'],
...
function sanitizeRealtimeProperties(value) {
  const output = Object.create(null);
  if (!value || typeof value !== 'object' || Array.isArray(value)) return { ...output };
  for (const [rawKey, rawValue] of Object.entries(value)) {
    const key = REALTIME_PROPERTY_KEYS.get(String(rawKey || '').toLowerCase());
    const number = Number(rawValue);
    if (!key || !Number.isFinite(number)) continue;
    output[key] = Math.max(0, Math.min(4, number));
  }
  return { ...output };
}
...
public static class MineradioWeParallaxPointerRelay {
  ...
  static bool ForwardPointer(IntPtr source, uint sourceProcessId, string expectedTitle,
      IntPtr sceneInput, IntPtr host, uint hostProcessId, string action, int detail, int xUnit, int yUnit) {
    ...
    if (String.Equals(action, "D", StringComparison.Ordinal)) {
      if (detail == 0) { message = WM_LBUTTONDOWN; wParam = MK_LBUTTON; }
      else if (detail == 1) { message = WM_MBUTTONDOWN; wParam = MK_MBUTTON; }
      else if (detail == 2) { message = WM_RBUTTONDOWN; wParam = MK_RBUTTON; }
      else return false;
    } else if (String.Equals(action, "U", StringComparison.Ordinal)) {
      if (detail == 0) message = WM_LBUTTONUP;
      else if (detail == 1) message = WM_MBUTTONUP;
      else if (detail == 2) message = WM_RBUTTONUP;
      else return false;
    } else if (String.Equals(action, "W", StringComparison.Ordinal)) {
      message = WM_MOUSEWHEEL;
      int wheelDelta = Math.Max(-1200, Math.Min(1200, detail));
      wParam = unchecked((wheelDelta & 0xffff) << 16);
    }
    if (!PostMessageW(sceneInput, WM_MOUSEMOVE, IntPtr.Zero, new IntPtr(packed))) {
      throw new Win32Exception(Marshal.GetLastWin32Error());
    }
    if (message != WM_MOUSEMOVE && !PostMessageW(sceneInput, message, new IntPtr(wParam), new IntPtr(packed))) {
      throw new Win32Exception(Marshal.GetLastWin32Error());
    }
    return true;
  }
  ...
  public static void Run(string sourceId, string expectedTitle, string expectedExecutable,
    ...
    try {
      ...
      while ((line = Console.ReadLine()) != null) {
        ...
        if (command.Length != 3 && command.Length != 4) continue;
        string action = command[0];
        bool move = String.Equals(action, "M", StringComparison.Ordinal);
        bool discrete = String.Equals(action, "D", StringComparison.Ordinal)
          || String.Equals(action, "U", StringComparison.Ordinal)
          || String.Equals(action, "W", StringComparison.Ordinal);
        if ((!move && !discrete) || (move && command.Length != 3) || (discrete && command.Length != 4)) continue;
        int detail = 0;
        int xUnit;
        int yUnit;
        int coordinateOffset = move ? 1 : 2;
        if ((!move && !Int32.TryParse(command[1], out detail))
            || !Int32.TryParse(command[coordinateOffset], out xUnit)
            || !Int32.TryParse(command[coordinateOffset + 1], out yUnit)
            || xUnit < 0 || xUnit > 65535 || yUnit < 0 || yUnit > 65535) continue;
        ForwardPointer(source, sourceProcessId, expectedTitle, sceneInput, host, hostProcessId, action, detail, xUnit, yUnit);
      }
    } finally {
      if (previousDpiContext != IntPtr.Zero) {
        try { SetThreadDpiAwarenessContext(previousDpiContext); } catch { }
...
public sealed class MineradioWeDwmSurfaceHost : Form {
  ...
  MineradioWeDwmSurfaceHost(IntPtr host, IntPtr source, string expectedTitle, int cornerRadius,
    ...
    Text = "Better Radio WE DWM Surface";
...
class WallpaperEngineRuntime {
  constructor(options = {}) {
    ...
    this.nativeTempPath = path.resolve(String(
      ...
      || path.join(process.env.LOCALAPPDATA || process.env.APPDATA || process.cwd(), 'Better Radio', 'native-helper-temp')
    ));
  ...
  async applyProperties(expectedSessionId = '', properties = {}) {
    ...
    const realtime = sanitizeRealtimeProperties(properties);
    if (!Object.keys(realtime).length) return { ok: true, applied: false, sessionId: session.sessionId };
    const payload = {
    ...
    await this._runTransientControl(session.executable, [
      ...
      'applyProperties',
      '-properties',
      `RAW~(${JSON.stringify(payload)})~END`,
      '-location',
      session.locationTitle,
    ]);
    return { ok: true, applied: true, sessionId: session.sessionId };
  }
  ...
  async getDwmGlassCaptureSource(expectedSessionId = '', options = {}) {
    ...
    if (options.allowDirectSourceId === true) {
      return {
        ...
        name: 'Better Radio WE DWM Surface',
    ...
    while (this.now() <= deadline) {
      ...
      const matched = sources.find((source) => {
        ...
        return !!match && match[1] === expectedWindowId
          && String(source && source.name || '') === 'Better Radio WE DWM Surface';
      });
  ...
  _postSessionPointerRelayEvent(session, eventType, detail, xUnit, yUnit) {
    if (!this._sessionPointerRelayCanPost(session) || session.parallaxPointerRelayBackpressured === true) return false;
    if (session.parallaxPointerRelayPending === true) this._flushSessionPointerRelay(session);
    const child = session.parallaxPointerRelayProcess;
    const stdin = child && child.stdin;
    try {
      const command = eventType === 'down' ? 'D' : (eventType === 'up' ? 'U' : 'W');
      const writable = stdin.write(`${command}:${detail}:${xUnit}:${yUnit}\n`, 'ascii');
      session.parallaxPointerRelayPosted += 1;
      if (writable === false) {
        session.parallaxPointerRelayBackpressured = true;
        const drain = () => {
  ...
  noteHostPointerActivity(expectedSessionId = '', coordinates = null) {
    ...
    const eventType = String(coordinates && coordinates.eventType || 'move').toLowerCase();
    if (!this._sessionPointerRelayCanPost(session)
      || !expectedSessionId
    ...
    if (eventType === 'down' || eventType === 'up') {
      const button = Math.round(Number(coordinates.button));
      if (button < 0 || button > 2) return false;
      return this._postSessionPointerRelayEvent(session, eventType, button, xUnit, yUnit);
    }
    if (eventType === 'wheel') {
      const delta = Math.max(-1200, Math.min(1200, Math.round(Number(coordinates.deltaY) || 0)));
      if (!delta) return false;
      return this._postSessionPointerRelayEvent(session, eventType, -delta, xUnit, yUnit);
    }
    if (eventType !== 'move') return false;
  ...
  async _prepareSilentLaunchFile(session, projectFile, scenePackage) {
    ...
    try {
      ...
      throw runtimeError('WALLPAPER_ENGINE_SILENT_STAGE_FAILED', error && error.message);
    }
  ...
  async prepareCaptureHost(expectedSessionId = '', host = {}) {
    const session = this.active;
    if (!session) throw runtimeError('WALLPAPER_ENGINE_NOT_ACTIVE');
    expectedSessionId = String(expectedSessionId || '');
    if (expectedSessionId && session.sessionId !== expectedSessionId) {
    ...
    session.parallaxPointerHostWindowId = String(host && host.hostWindowId || '');
    session.parallaxPointerHostExecutable = String(host && host.hostExecutable || '');
    const result = await this.parkActiveWindow(session.sessionId);
    result.pointerRelayReady = await this._startSessionPointerRelay(session).catch(() => false);
    session.captureAttached = true;
    this._scheduleSessionMuteReassertions(session);
    return result;
  }
  ...
  async start(id, options = {}) {
    ...
    const session = {
      ...
      sessionId,
      locationTitle: `Better Radio Wallpaper ${sessionId}`,
      sourceId: '',
      windowSourceId: '',
