...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for Better Radio icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || 'Better Radio';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`Better Radio executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`Better Radio icon was not found: ${iconPath}`);

  const version = context.packager.appInfo.version;
  console.log(`  • injecting Better Radio resources  rcedit=${rceditPath}`);
  execFileSync(rceditPath, [
    exePath,
    '--set-icon', iconPath,
    '--set-version-string', 'FileDescription', 'Better Radio',
    '--set-version-string', 'ProductName', 'Better Radio',
    '--set-version-string', 'CompanyName', 'QingYaoSheep',
