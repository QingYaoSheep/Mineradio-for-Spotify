...
async function main() {
  ...
  try {
    ...
    assert(pakScene && pakScene.enginePlayable, 'valid Scene packages should be available to the guarded native runtime');
    assert(customPakSceneBeforeImport && customPakSceneBeforeImport.previewOnly, 'non-standard package names require explicit package import');
    ...
    assert(customPakScene && customPakScene.enginePlayable, 'manual project.json import should expose a validated native Scene target');
    await assert.rejects(
      () => instance.addManualProjectFile(path.join(invalidPakProject, 'resources.pak')),
      /所选文件不是有效的 Wallpaper Engine PKGV 场景包/
    );
    ...
    assert(savedConfig.manualRoots.includes(customPakProject));
